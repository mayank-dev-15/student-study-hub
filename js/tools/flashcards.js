(function(){
  function render(c){
    ...
    <div class="flashcard-container"><div class="flashcard" id="fc-card" onclick="FLC.flip()"><div class="flashcard-face flashcard-front" id="fc-front"></div><div class="flashcard-face flashcard-back" id="fc-back"></div></div></div>
    ...
    <div style="text-align:center;margin-top:12px"><button class="btn btn-secondary btn-sm" onclick="FLC.exit()">Exit</button></div></div>`;
    FLC.renderList();
  }
  window.FLC={
    cards:Store.get('flashcards',[]),idx:0,queue:[],
    add(){const f=document.getElementById('fc-f').value.trim(),b=document.getElementById('fc-b').value.trim();if(!f||!b){Toast.error('Fill both sides');return;}this.cards.push({f,b,iv:0,nr:Date.now(),created:Date.now()});Store.set('flashcards',this.cards);document.getElementById('fc-f').value='';document.getElementById('fc-b').value='';FLC.renderList();Toast.success('Added!');},
    ...
    renderList(){const el=document.getElementById('fc-list');if(!this.cards.length){el.innerHTML='<div class="empty-state"><p>No cards yet.</p></div>';return;}
      ...
      el.innerHTML=`<div class="table-wrap"><table><thead><tr><th>Front</th><th>Back</th><th>Lvl</th><th></th></tr></thead><tbody>${rows}</tbody></table><button class="btn btn-primary" style="margin-top:12px" onclick="FLC.start()">📖 Study (${this.cards.length})</button></div>`;},
