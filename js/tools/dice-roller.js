(function(){
  ...
  window.Dice={history:[],roll(d){let r=Math.floor(Math.random()*d)+1;document.getElementById('dice-result').textContent=r;this.history.unshift('d'+d+' → '+r);if(this.history.length>20)this.history.pop();document.getElementById('dice-history').innerHTML=this.history.map(h=>`<div>${h}</div>`).join('');}};
