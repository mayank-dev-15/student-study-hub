(function(){
  ...
  window.LI={
    gen(){let p=parseInt(document.getElementById('li-p').value)||3,w=parseInt(document.getElementById('li-w').value)||50;let out='';for(let i=0;i<p;i++){let para='';for(let j=0;j<w;j++)para+=words[Math.floor(Math.random()*words.length)]+' ';out+=para.trim()+'

';}document.getElementById('li-out').value=out.trim();}
  };
  Router.registerRoute('#lorem-ipsum','Lorem Ipsum',render);
})();